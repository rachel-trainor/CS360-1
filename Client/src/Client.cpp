...
void Client::parseResponse() {
	...
	size_t index = readToSentinel(' ', this->response);
	string command = this->response.substr(0, index);

	if (command != "OK\n") {
		if (this->response.size() > index + 1) {
			this->response = this->response.substr(index + 1);
...
void Client::createReadResponse() {
	...
	//read the subject
	size_t index = readToSentinel(' ', this->response);
	string subject = this->response.substr(0, index);
	if (this->response.size() > index + 1) {
		this->response = this->response.substr(index + 1);
	}
	...
	//read to \n to grab message
	index = readToSentinel('\n', this->response);
	string message = this->response.substr(0, index);
	if (this->response.size() > index + 1) {
		this->response = this->response.substr(index + 1);
	}

	finalResponse += subject;
	finalResponse += "\n";
...
bool Client::get_response() {
	...
	if (debug) {
		cout << "Client received response: " << response << endl;
	}
